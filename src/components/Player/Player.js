...
class Player extends Component {
	...
	handleChipChange = (chip) => {
		switch (chip) {
			case 'Delta':
				this.setState({ beat: 2, activeChip: 'Delta' });
				break;
			case 'Theta':
				this.setState({ beat: 5, activeChip: 'Theta' });
				break;
			case 'Alpha':
				this.setState({ beat: 10, activeChip: 'Alpha' });
				break;
			case 'Beta':
				this.setState({ beat: 24, activeChip: 'Beta' });
				break;
			case 'Gamma':
				this.setState({ beat: 63, activeChip: 'Gamma' });
				break;
			default:
				console.log(chip);
		}
	...
	componentWillUnmount() {
		if (this.soundTimerInterval) clearInterval(this.soundTimerInterval);
	}
